...
async function sendEmails(subscriberEmail: string): Promise<void> {
  ...
  // 2. Confirmation to subscriber (best-effort — don't fail the whole flow if this fails)
  if (TEMPLATE_CONFIRM) {
    try {
      await emailjs.send(
        SERVICE_ID,
        ...
        {state === 'success' ? (
          ...
          >
            ...
            <p className="text-text-secondary max-w-md mx-auto">
              A confirmation has been sent to <span className="text-text-primary font-mono">{email}</span>. Here's what happens next:
            </p>
            <ul className="max-w-sm mx-auto text-left space-y-3 text-sm text-text-secondary">
              <li className="flex items-start gap-3">
                ...
                <span><strong className="text-text-primary">Weekly digest every Monday</strong> — top stories across AI, cybersecurity, gadgets, space &amp; gaming, curated and ranked.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-neon-cyan mt-0.5 shrink-0">●</span>
                <span><strong className="text-text-primary">No noise.</strong> No sponsored content, no filler — only stories that actually matter.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-neon-cyan mt-0.5 shrink-0">●</span>
                <span><strong className="text-text-primary">Breaking alerts</strong> when something major drops mid-week.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-neon-cyan mt-0.5 shrink-0">●</span>
                <span>Unsubscribe anytime — one click, no questions asked.</span>
              </li>
            </ul>
          </motion.div>
        ) : (
